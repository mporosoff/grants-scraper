...
def judge_items(request):
    if request.get("protocol") == "post-audit-complete-v1":
        from tools.team_recommender_post_audit_executor import judge_items as post_items
        return post_items(request)
...
def claimed_judge_items(row, historical=None):
    if not row.get('purpose','').startswith(('d1-','d2-','d3-','s3-','post-audit')):
        return []
