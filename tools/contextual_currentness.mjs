...
const sources=job.release_id===phase2.release_id?JSON.parse(fs.readFileSync('config/contextual_team/phase2-source-inputs-v2.json','utf8')):inputs;
const scope=sources.scopes.find(s=>s.id===job.scope_id);
if(!scope||![inputs.snapshot_id,option1.release_id,phase2.release_id].includes(job.release_id))throw Error('contextual_job_not_in_snapshot');
