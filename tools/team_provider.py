...
def contract():
    return {stage: stage_contract(stage) for stage in routes()}


def stage_prompt(stage, settings=None):
    from scripts import build_opportunity_teams as teams
    settings = settings or config()
    historical = {'decomposition': teams.DECOMPOSE, 'adjudication': teams.ADJUDICATE, 'verification': teams.VERIFY}
    return settings.get('production_prompts', {}).get(stage, historical[stage])


def stage_contract(stage, settings=None):
    """The actual production request, also used by finite qualification."""
    settings = settings or config()
    return {'route': routes(settings)[stage], 'settings': stage_settings(stage, settings),
            'schema': schemas()[stage], 'prompt': stage_prompt(stage, settings)}
...
def request(provider, prompt, data):
    ...
    if not durable.exists() and optional.exists():
        try:
            ...
            pass
    value = provider.offline.json(route, stage, selected['prompt'], data, schema,
