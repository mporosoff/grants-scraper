...
async function main() {
  ...
  const parentIds = new Set(c.GRANT_CATALOG.opportunities.map(r => r.opportunity_id));
  ...
  const childIds = new Map(children.opportunities.map(r => [r.opportunity_id, r.parent_id]));
  const supported = historical.filter(s => parentIds.has(s.parent_id) && (s.record_type === 'specific_parent' && s.id === s.parent_id || s.record_type === 'publishable_child' && childIds.get(s.id) === s.parent_id));
  const records = new Map([...c.GRANT_CATALOG.opportunities, ...children.opportunities].map(r => [r.opportunity_id, r]));
  const scopes = supported.map(s => ({...s, scope_label: records.get(s.id).title || s.id}));
  ...
  fs.writeFileSync('docs/team-recommender/profile-repair/shared-candidate-index.json', JSON.stringify(pack.index, null, 2) + '\n');
  fs.writeFileSync('docs/team-recommender/profile-repair/shared-package-receipt.json', JSON.stringify({generation: pack.index.generation_id, registry_generation: pack.packet.registry_generation,
    ...
    historical_scope_inventory: historical.length, canonical_scopes: scopes.length, inventory_source: 'Existing public availability IDs only; no saved team or judge file is read', omitted: historical.filter(s => !supported.includes(s)).map(s => ({id: s.id, reason: 'No identical canonical parent or publishable child projection'})),
