...
def check_reservation(state,provider,metadata,amount,input_tokens,output_tokens):
    if metadata.get('purpose','').startswith('cb-fc-cat-'):
        from tools.catalog_correction_policy import check_reservation as catalog_check
        return catalog_check(state,provider,metadata,amount,input_tokens,output_tokens)
    if metadata.get('purpose','').startswith('cb-fc-i3c-'):
        from tools.contextual_team_iteration3_continuation_policy import check_reservation as continuation_check
        return continuation_check(state,provider,metadata,amount,input_tokens,output_tokens)
    ...
    if metadata.get('purpose','').startswith('cb-fc-'):
        from tools.contextual_team_completion_check import check_reservation as completion_check
        return completion_check(state,provider,metadata,amount,input_tokens,output_tokens)
    if metadata.get('purpose')=='cb-cc-wire-repair':
        from tools.contextual_team_wire_repair import check_reservation as wire_check
        return wire_check(state,provider,metadata,amount,input_tokens,output_tokens)
    ...
    if metadata.get('purpose','').startswith('cb-lr-'):
        from tools.contextual_team_latency_policy import check_reservation as latency_check
        return latency_check(state,provider,metadata,amount,input_tokens,output_tokens)
    ...
    if metadata.get('purpose','').startswith('cb-p1-'):
        from tools.contextual_team_phase1 import check_reservation as phase1_check
        return phase1_check(state,provider,metadata,amount,input_tokens,output_tokens)
