...
def main():
 ...
 for cohort in ['legacy','rollout50','rollout150']:
  folder=ROOT/'outputs/team-recommender-post-audit'/('release-'+cohort+'-'+sha[:12]);stage=folder/'input';bundle=folder/'candidate'
  ...
  result[cohort]={'candidate_id':built['candidate_id'],'assembly_sha':sha,'bundle':str(bundle.relative_to(ROOT)).replace('\\','/'),'files':len(built['files']),'scope_generation':index['generation_id'],'scopes':len(index['scopes']),'activated':False,'new_generation_calls':0}
