...
from tools.offline_spend import encoded, normalize_usage, cost_microusd, error_diagnostics
...
# Preserved measured transport/accounting adapter; production uses the active
# shared client in run_budgeted_documents. This is not a new evaluation route.
def historical_cov4_instrument(ledger, cache, classify):
...
def cov4(ledger, directory, protocol):
    ...
    bounded = historical_cov4_instrument(ledger, directory / 'cov4-cache', gate.historical_classify_fundability)
    rows = []
