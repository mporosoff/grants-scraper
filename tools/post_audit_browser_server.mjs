...
const server=http.createServer((req,res)=>{try{
 ...
 const cohort=req.headers['x-post-audit-cohort']||'static';if(!['static','legacy','rollout50','rollout150'].includes(cohort))return res.writeHead(400).end();
 ...
 const packages='docs/team-recommender/post-audit/release-packages-v1.json';
 if(cohort!=='static'&&fs.existsSync(packages)){const pkg=JSON.parse(fs.readFileSync(packages)).packages[cohort];const file=path.join(root,pkg.bundle,'files',p);const raw=fs.readFileSync(file),body=/gzip/.test(req.headers['accept-encoding']||'')?gzipSync(raw):raw;res.writeHead(200,{'Content-Type':{'.html':'text/html','.js':'text/javascript','.css':'text/css','.json':'application/json','.svg':'image/svg+xml'}[path.extname(p)]||'application/octet-stream','Content-Length':body.length,'Cache-Control':'no-store',...(body!==raw?{'Content-Encoding':'gzip'}:{})});return res.end(req.method==='HEAD'?undefined:body);}
 ...
 const mime={'.html':'text/html','.js':'text/javascript','.css':'text/css','.json':'application/json','.svg':'image/svg+xml','.woff2':'font/woff2'}[path.extname(p)]||'application/octet-stream';
