...
const receiptDir=process.env.TEAM_VALIDATION_RECEIPTS||'post-audit';if(!['post-audit','evidence-repair'].includes(receiptDir))throw Error('Unknown local validation receipt directory');
const root=process.cwd(),receipt=JSON.parse(fs.readFileSync(`docs/team-recommender/${receiptDir}/routing-inventory-v1.json`));
const server=http.createServer((req,res)=>{try{
 ...
 const packages=[4,3,2,1].map(v=>`docs/team-recommender/${receiptDir}/release-packages-v${v}.json`).find(p=>fs.existsSync(p));
