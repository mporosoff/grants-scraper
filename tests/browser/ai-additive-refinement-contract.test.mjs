...
test("runtime owns a separate refinement overlay, stale identity checks, exact restore, and bounded chat", () => {
  ...
  assert.doesNotMatch(chatIds, /slice\(/, "The eligible scope must include records beyond the first ten");
  assert.match(resultsChat, /eligibleIds.length > MAX_CHAT_SCOPE/);
  assert.match(resultsChat, /await retrieveChatContext\(cleanQuestion, eligibleIds, requestMessages\)/);
