...
test('32 deterministic small-pool cases agree with independent Python exact optimization',()=>{
 ...
 const cases=Array.from({length:32},()=>({member_quality_floor:n.PARAMETERS.memberQuality,weights:[.7,.1,.1,.1],rows:Array.from({length:8},(_,i)=>({id:String(i),anchor:i%3===0,automatic_quality:next(),scores:Array.from({length:4},()=>next())}))}));
 ...
 cases.forEach((c,i)=>{const rows=c.rows.map(r=>({id:r.id,automatic_quality:r.automatic_quality,edges:r.scores.map(score=>({score,admitted:true,core:r.anchor?.7:.1}))}));
...
test('deliberate 2/3/4 size selection and every final marginal contribution',()=>{
 ...
 for(let size=2;size<=4;size++){
  const weights=[.7,...Array(size-1).fill(.3/(size-1))],rows=Array.from({length:size},(_,j)=>({id:String(j),automatic_quality:.8,edges:weights.map((_,i)=>({score:Number(i===j),admitted:i===j,core:j===0?1:.3}))}));
...
test('duplicate evidence and aspect splitting do not buy utility; matched-size A/B and MMR comparison',()=>{
 const n=runtime().TeamRecommender;
 const rows=Array.from({length:10},(_,i)=>({id:String(i),automatic_quality:.8,baseline:i%2===0?1:.2,edges:[0,1].map(j=>({score:i%2===j?.9:0,admitted:i%2===j,core:.7}))}));
 ...
 const mmr=n.optimize(m,[],{mmr:.1});assert.deepEqual(mmr,b);assert.ok(mmr.options.every(t=>t.score>=.95*b.maximum));
});
