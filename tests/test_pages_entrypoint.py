...
class GitHubPagesEntrypointTests(unittest.TestCase):
    ...
    def test_match_explorer_supports_public_search_and_optional_ai(self):
        ...
        self.assertIn('id="chat-provider-slot"', explorer_html)
        ...
        self.assertRegex(explorer_html, r'<dialog[^>]+id="result-assistant"[^>]+data-shell-drawer')
        ...
        self.assertIn('SiteShell.openDrawer(dialog, opener', application_js)
        self.assertIn('SiteShell?.closeDrawer($("result-assistant")', application_js)
    ...
    def test_scheduled_refresh_has_health_checks_and_failure_alert(self):
        ...
        self.assertIn("uses: ./.github/actions/release-setup", workflow)
        self.assertIn("tools.validate_release_candidate", workflow)
        self.assertNotIn("web/tests/", workflow)
        self.assertIn("python -m scripts.update_catalog_docs", workflow)
        self.assertIn("tools.publish_release_candidate", workflow)
