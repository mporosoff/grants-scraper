...
export async function fixture(extra = {}) {
  ...
  const c = context(extra), record = {opportunity_id: 'call', title: 'Catalysis spectroscopy and membranes', description: 'Heterogeneous catalysis uses spectroscopy and membranes.', status: 'posted', close_date: '2026-09-12'};
  ...
  const directory = {schema_version: 1, registry_generation: 'a'.repeat(64), researchers: Array.from({length: 12}, (_, i) => {
    const term = ['heterogeneous catalysis', 'spectroscopy', 'membranes'][i % 3];
