...
class CompactCheckerContract(unittest.TestCase):
    ...
    def test_retained_assessment_replays_to_exact_canonical_identity(self):
        receipt=json.loads(Path('docs/team-recommender/contextual-stage-b/luna-repair-closeout-v1.json').read_bytes())
        keys=('coverage','claim_refs','source_ref','reason','gap')
        wire={'decisions':{p['person_id']:{d['role_id']:{k:d[k] for k in keys}
            for d in p['decisions']} for p in receipt['people']}}
        value=pairs.resolve(wire,self.data)
        self.assertEqual(identity(self.data),receipt['input_sha256'])
        self.assertEqual(identity(value),receipt['assessment_value_identity'])
        self.assertEqual(pairs.validate_cached(value,self.data),value)
        self.assertEqual(sum(len(p['decisions']) for p in value['people']),24)
        self.assertEqual(value['state'],'complete_unverified_assessment')
