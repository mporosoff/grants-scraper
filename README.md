...
“Ask AI” is enabled after a keyword or profile search or any non-default filter is applied, and uses up to the ten most relevant current opportunities. Strong matches precede Potential matches; relevance within each tier is independent of the card sort and pagination, with the usual deadline ordering when there are no search or profile terms. Questions and follow-ups use the same ranking, with question-relevant description excerpts and matched child-topic evidence. Scope labels disclose when only the top results are included. Browsing the catalog with default filters keeps Ask AI disabled until search criteria are applied. Change the search or filters to discuss different opportunities, or use a card’s More menu to discuss one opportunity. The main Funding Finder and Team Match searches continue using their full catalog local and semantic pipelines. Single-opportunity and uploaded-notice chat remain available independently of the general result count. The mobile composer stays within the visible viewport as the keyboard opens.
...
September 14, 2026 build contains 1,416 current funding opportunities (1,075 posted and
341 forecasted) from ARPA-H (10), DARPA / IARPA research solicitations (4), DOE EERE
Exchange (1), Grants.gov (1,310), NASA ROSES (3), NYSERDA (37), National Science
Foundation (11), U.S. National Science Foundation (1), VPR funding digest (limited
submissions & foundations) (39), with no deadline before the catalog date. It provides a
direct official announcement for 277 records, an official source-page route for another
630, and the official Grants.gov record for the remaining 509. Across all route types,
729 records also contain an official source URL.
<!-- catalog-stats:end -->
