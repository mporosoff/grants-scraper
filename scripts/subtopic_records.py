...
DISPLAY_RECORD_LIMIT_BYTES = 2 * 1024
DISPLAY_RECORD_FIELDS = frozenset({
    "record_type",
    "child_type",
    "subtopic_id",
    "opportunity_id",
...
def _fit_term_display_to_record_budget(record, term_frequencies=None):
    """Keep as many readable term labels as fit the 2 KiB display contract."""
    display = record.get("term_display") or {}
    frequencies = term_frequencies or {}
    items = sorted(
        display.items(),
        key=lambda item: (-int(frequencies.get(item[0], 0)), item[0]),
    )[:MAX_TERM_DISPLAY]
    projection = {
        key: value for key, value in record.items()
        if key in DISPLAY_RECORD_FIELDS
    }

    def serialized_size(count):
        projection["term_display"] = dict(items[:count])
    ...
    while low < high:
        middle = (low + high + 1) // 2
        if serialized_size(middle) <= DISPLAY_RECORD_LIMIT_BYTES:
            low = middle
        else:
            high = middle - 1
    record["term_display"] = dict(items[:low])
...
def sidecar_payload(cache, *, approvals=None):
    ...
    for parent, entry in sorted((cache.get("records") or {}).items()):
        ...
        for child in sorted(
            ...
            _fit_term_display_to_record_budget(stored, terms)
