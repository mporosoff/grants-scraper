        ...
        try:
            ...
            results.append(
                AdapterResult(
                    ...
                    snapshot_complete=adapter.snapshot_complete,
                )
            )
            continue
        results.append(
            AdapterResult(
                ...
                display_name=adapter.display_name,
                source_type=adapter.source_type,
                ok=not bool((adapter.diagnostics or {}).get("partial_failure")),
                error=str(adapter.diagnostics.get("failure_reason") or "One or more official source partitions failed") if (adapter.diagnostics or {}).get("partial_failure") else None,
