(function (global) {
  ...
  // Restricted I2: preserve one initialized module for one exact SRI identity.
  // Reinitializing the client discards its validated graph and pending-work caches.
  var contextualRuntimeLoads = new Map();
  function contextualRuntime(name, hash, exported) {
    var previous = contextualRuntimeLoads.get(name);
    if (previous && previous.hash === hash &&
        (!previous.api || previous.api === global[exported])) return previous.promise;
    var entry = {hash: hash, api: null};
    contextualRuntimeLoads.set(name, entry);
    // Serialize replacements so a slower old script cannot overwrite a new one.
    entry.promise = (previous ? previous.promise.catch(function () {}) : Promise.resolve())
      .then(function () { return injectRuntime(name, hash); })
      .then(function () {
        if (!global[exported]) throw new Error("Contextual runtime unavailable.");
        entry.api = global[exported];
      }).catch(function (error) {
        if (contextualRuntimeLoads.get(name) === entry) contextualRuntimeLoads.delete(name);
        throw error;
      });
  ...
  async function loadContextual(index,options){
    await contextualRuntime("contextual-team-engine",index.runtime?.contextual_engine,"ContextualTeamEngine");
    await contextualRuntime("contextual-team-client",index.runtime?.contextual_client,"ContextualTeamClient");
