...
const PROGRAM_OFFICER_SOURCES = ["NSF", "NIH", "DOE"];
...
function sourceSummary(payload) {
  return {
    ...
    ...(payload.contact_post_validation ? { contact_post_validation: payload.contact_post_validation } : {}),
...
function validateSnapshotCreate(body, config, asOf = new Date()) {
  ...
  if (criteria.publicCriteria.mode === "program_officer"
      && (sources.length !== 1 || !PROGRAM_OFFICER_SOURCES.includes(sources[0]))) return null;
  if (criteria.publicCriteria.program_office && (sources.length !== 1 || sources[0] !== "DOE")) return null;
  return {
    sources,
    ...
    if (path === "/health" && request.method === "GET") {
      ...
      return json(origin, serviceReady ? 200 : 503, {
        ...
        complete_result_snapshots: {
          ...
          program_officer_sources: PROGRAM_OFFICER_SOURCES,
          program_officer_evidence: {
            ...
            indexed_abstract_characters_per_record: SNAPSHOT_EVIDENCE_INDEXED_ABSTRACT_LIMIT,
