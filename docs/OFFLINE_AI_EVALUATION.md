...
The ordinary receipt records the manual browser requirement before the browser step starts. Failed or interrupted browser validation blocks publication-only retries; rerun manual validation with `final_integration=true` to reuse ordinary checks and finish that gate. Browser identity includes relative repository imports transitively, including the award snapshot and ROR modules used by the suite.

Explicit `validate` and `publish` resumes recover the latest validation artifact for the exact named candidate even when `receipt_run` is omitted or points to an older attempt. A failed ordinary report also retains the requested browser gate. The current protected candidate pointer never substitutes for the named candidate.
