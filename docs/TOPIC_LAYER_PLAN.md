...
Status: in progress · Version 8.35 · Written 2026-08-15 · **Revised 2026-08-20 through P9.0/P9.1 and the blocked first-current-cache gate**

> **Start at §18, and read §18.0 first.** §18 defines the minimum path — the **eleven** work packages **P1 … P11** — and lists what is deferred and what it costs. **§18.0 is the canonical namespace**: package IDs, the `BUG-*` / `MEAS-*` / `DEC-*` / `DEBT-*` prefixes, the migration table for every legacy label (`Package A–G`, `D½`, `D⅝`, `D¾`, `S1–S3`, `Package N`, bare `D#`, `M#`), the current ordered path, and a diagram. **P5, P6, P7, P9.0, P9.1 and MEAS-8 are CLOSED; P8 is complete. P9 remains OPEN at P9.2's first-current-cache gate.** The sidecar/scoring/currentness code is present, but the new ARPA-H parent source has not yet been materialized into the catalog, and `data/subtopics.js` / `evaluation/p9_backfill_results.json` are deliberately absent. The corrected live work was rejected by the external execution-usage limit through **2026-08-27 09:36 America/New_York**, after the earlier completed traversal was correctly withheld for a source-hash validation defect. A fail-closed preflight now prevents the provisional 1,405-parent frame from being mistaken for the post-ARPA-H final frame. **P10 and P11 implementation have not started.** **§18.0.6 is the canonical Open Obligations Register** — every unresolved `DEC-*`, `BUG-*`, `DEBT-*` and `MEAS-*` in the project, each with an owner and a gate. §10's four phases remain as background; §18 supersedes them as the unit of work, and §15 tracks §18.
>
...
>
> P9.2's deterministic sidecar schema, compact subject-only index, parent consistency validator and resumable implementation-hash checkpoint are implemented; P9.3's sole currentness rule is parent membership. The committed frame measures the **pre-ARPA-H-generated catalog** at **1,405 current / 70 excluded** and is now explicitly provisional: the catalog must first materialize the new official parent source, then the frame must be regenerated. The first live campaign traversed those 1,405 parents but wrote no output because final validation caught 14 referenced TDAC children without source hashes. That defect is fixed. The corrected network work was then blocked before execution by the external usage limit; the user-scope classifier credential was also absent, so generic candidates would fail closed even when transport resumes. **No partial or invalid sidecar is committed. P9 remains OPEN; do not start P10.**
>
> Offline closeout verification is green with real exit codes: **796 unit tests passed, 1 skipped; 93 browser tests passed; 37-query flag-off baseline zero top-10 churn; 50-case P9 scoring artifact byte-identical and cardinality-invariant; `verify_no_drift` 22/22 protected artifacts unchanged.** These checks do not substitute for the blocked live sidecar gate.
>
...
| **P9** | Storage and scoring | ⚠️ **OPEN at the first-current-cache gate.** P9.0 and P9.1 are closed; P9.2's contract and P9.3 parent-only currentness are implemented. The official ARPA-H rows must first be materialized into the catalog and the provisional 1,405-parent frame regenerated; the required live work is externally blocked through 2026-08-27 09:36 America/New_York. No validated sidecar/results artifact exists |
| **P10** | Retrieval and UI | ⛔ **not started** |
| **P11** | Enable and ship | ⛔ **not started.** The only package that flips a flag (§0.4 rule 9) |

**Gates that stop work by design:** ~~P4's gate~~ (**reconciled at P7 closeout: final 42% < 50%, target not lowered**) · ~~P5's Cov4 gate~~ (**passed 2026-08-26**) · ~~P7 behind P5 and P6~~ (**P7 closed**) · **P9's validated first current sidecar/results pair**, currently blocked by the external execution-usage limit · **P11 cannot close while §18.0.6 holds an open obligation.**
...
- [ ] **P9.2. Implement the settled sidecar and create the first current cache.** ⚠️ Schema, compact index, generator, validator and resumable checkpoint are implemented. The committed **1,405-current / 70-excluded** frame is provisional because it predates materialization of the newly admitted ARPA-H source; a source-floor preflight now refuses it. **The parent catalog must be regenerated, the frame re-frozen, and then `data/subtopics.js` / `evaluation/p9_backfill_results.json` built.** That live work is externally blocked through 2026-08-27 09:36 America/New_York
- [x] **P9.3. Minimal currentness only.** ✅ A child is current iff its parent belongs to the regenerated frozen current population. No child status/deadline/archive semantics were added. The provisional committed-catalog frame measured 1,405 current and 70 excluded (64 expired, 6 archived); those counts are not promoted as the final post-ARPA-H frame
- [ ] **GATE:** ⚠️ P9.1 scoring, flag-off baseline and parent-only currentness sub-gates pass, but the package gate cannot pass until the validated first-current sidecar/results pair exists. **P9 remains OPEN; P10 must not start**
...
| **P9** | Storage and scoring | Package E | ⚠️ **open at first-current-cache gate; P9.0/P9.1 closed** |
...
| P9 | Storage and scoring | Package E | **open at first-current-cache gate** |
| P9.0–P9.3 | Re-key identity · scoring prototype · implement · currentness | E0, E1, E2, E3 | **P9.0/P9.1 closed; P9.2 contract built but live artifact blocked; P9.3 built** |
| P10 | Retrieval and UI | Package F | not started |
...
| 12 | **P9** — subtopic-aware storage and scoring | ⚠️ **OPEN.** P9.0/P9.1 closed; identity, source routes, scoring prototype, sidecar contract and parent-only currentness implemented. The committed 1,405-current / 70-excluded frame predates ARPA-H catalog materialization and is provisional; preflight now refuses that omission. Catalog regeneration, frame re-freeze and the corrected first-current-cache run are externally blocked through 2026-08-27 09:36 America/New_York; output artifacts remain absent. **Do not start P10** |
...
| Implement the settled sidecar | ⚠️ Schema/index/generator/validator are implemented. ARPA-H catalog materialization, frame re-freeze and the corrected live build are externally blocked. No `data/subtopics.js` or results artifact exists, so this row and P9 remain open |
| **Minimal currentness only** | ✅ Implemented: a subtopic is current **iff its parent is in the frozen current population**. Nothing else. No `own_deadline`, no child `closed`/`expired`/`removed` states, no archive |

**Gate:** scoring and flag-off query sub-gates pass; the package gate remains red until a validated current sidecar/results pair is generated and committed. **P9 remains OPEN.**
