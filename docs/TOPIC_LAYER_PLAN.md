...
> **Start at §18, and read §18.0 first.** §18 defines the minimum path — the **eleven** work packages **P1 … P11** — and lists what is deferred and what it costs. **§18.0 is the canonical namespace**: package IDs, the `BUG-*` / `MEAS-*` / `DEC-*` / `DEBT-*` prefixes, the migration table for every legacy label (`Package A–G`, `D½`, `D⅝`, `D¾`, `S1–S3`, `Package N`, bare `D#`, `M#`), the current ordered path, and a diagram. **P5, P6, P7, P9.0, P9.1 and MEAS-8 are CLOSED; P8 is complete. P9 remains OPEN at P9.2's first-current-cache gate.** The sidecar/scoring/currentness code is present, but `data/subtopics.js` and `evaluation/p9_backfill_results.json` are deliberately absent: the corrected live rerun was rejected by the external execution-usage limit through **2026-08-27 09:36 America/New_York**, after the earlier completed traversal was correctly withheld for a source-hash validation defect. **P10 and P11 implementation have not started.** **§18.0.6 is the canonical Open Obligations Register** — every unresolved `DEC-*`, `BUG-*`, `DEBT-*` and `MEAS-*` in the project, each with an owner and a gate. §10's four phases remain as background; §18 supersedes them as the unit of work, and §15 tracks §18.
>
...
>
> P9.2's deterministic sidecar schema, compact subject-only index, parent consistency validator, frozen current-parent frame and resumable implementation-hash checkpoint are implemented; P9.3's sole currentness rule is parent membership in the **1,405-current / 70-excluded** frozen frame. The first live campaign traversed all 1,405 parents but wrote no output because final validation caught 14 referenced TDAC children without source hashes. That defect is fixed. The corrected network run was then blocked before execution by the external usage limit; the user-scope classifier credential was also absent, so generic candidates would fail closed even when transport resumes. **No partial or invalid sidecar is committed. P9 remains OPEN; do not start P10.**
>
> Offline closeout verification is green with real exit codes: **794 unit tests passed, 1 skipped; 93 browser tests passed; 37-query flag-off baseline zero top-10 churn; 50-case P9 scoring artifact byte-identical and cardinality-invariant; `verify_no_drift` 22/22 protected artifacts unchanged.** These checks do not substitute for the blocked live sidecar gate.
>
...
| **P9** | Storage and scoring | ⚠️ **OPEN at the first-current-cache gate.** P9.0 and P9.1 are closed; P9.2's contract and P9.3 parent-only currentness are implemented. The corrected 1,405-parent live run is externally blocked through 2026-08-27 09:36 America/New_York, so no validated sidecar/results artifact exists and P9 cannot close |
...
- [ ] **P9.2. Implement the settled sidecar and create the first current cache.** ⚠️ Schema, compact index, generator, validator, frame and resumable checkpoint are implemented. **The required `data/subtopics.js` and `evaluation/p9_backfill_results.json` are absent** because the corrected live run is externally blocked through 2026-08-27 09:36 America/New_York. Never commit the invalid/partial predecessor
- [x] **P9.3. Minimal currentness only.** ✅ A child is current iff its parent belongs to the frozen current population: **1,405 current, 70 excluded (64 expired, 6 archived)**. No child status/deadline/archive semantics were added
...
| DEC-19 | bounded public ARPA-H source | yes | 10 current official rows; 11 frozen reconciled; 3 frozen now closed | no |
...
| 12 | **P9** — subtopic-aware storage and scoring | ⚠️ **OPEN.** P9.0/P9.1 closed; identity, source routes, scoring prototype, sidecar contract and parent-only currentness implemented. The frozen frame is 1,405 current parents / 70 excluded. The corrected first-current-cache run is externally blocked through 2026-08-27 09:36 America/New_York; `data/subtopics.js` and `evaluation/p9_backfill_results.json` remain absent by design. **Do not start P10** |
...
| Implement the settled sidecar | ⚠️ Schema/index/generator/validator are implemented, but the corrected live build is externally blocked. No `data/subtopics.js` or results artifact exists, so this row and P9 remain open |
