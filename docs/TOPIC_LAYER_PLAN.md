...
Status: in progress · Version 8.36 · Written 2026-08-15 · **Revised 2026-08-21 through P9 closeout**

> **Start at §18, and read §18.0 first.** §18 defines the minimum path — the **eleven** work packages **P1 … P11** — and lists what is deferred and what it costs. **§18.0 is the canonical namespace**: package IDs, the `BUG-*` / `MEAS-*` / `DEC-*` / `DEBT-*` prefixes, the migration table for every legacy label (`Package A–G`, `D½`, `D⅝`, `D¾`, `S1–S3`, `Package N`, bare `D#`, `M#`), the current ordered path, and a diagram. **P5, P6, P7, P9 and MEAS-8 are CLOSED; P8 is complete. NEXT: P10.** P9 materialized the bounded ARPA-H parent source, froze the final **1,415-current / 70-excluded** campaign frame, and committed the validated deterministic sidecar/results pair. Production feature flags remain off, and **P10 and P11 implementation have not started.** **§18.0.6 is the canonical Open Obligations Register** — every unresolved `DEC-*`, `BUG-*`, `DEBT-*` and `MEAS-*` in the project, each with an owner and a gate. §10's four phases remain as background; §18 supersedes them as the unit of work, and §15 tracks §18.
>
...
**Gates that stop work by design:** ~~P4's gate~~ (**reconciled at P7 closeout: final 42% < 50%, target not lowered**) · ~~P5's Cov4 gate~~ (**passed 2026-08-26**) · ~~P7 behind P5 and P6~~ (**P7 closed**) · ~~P9's validated first current sidecar/results pair~~ (**passed 2026-08-21**) · **P11 cannot close while §18.0.6 holds an open obligation.**
...
| **DEC-18** | Typed children — DEC-11's "C later" half | *(new in 8.29)* | **P9 half closed: explicit `child_type`, v1 `subject` only; future types deferred to P10** |
...
| 13 | **P10** — retrieval and UI | **NEXT.** **MEAS-9** post-P9 manual-profile / CV / ORCID relevance regression, including the real ORCID validation · **MEAS-10** the multi-researcher pilot · MEAS-5 · retrieval and UI work · due `DEC-*`: DEC-3, DEC-4, DEC-5, DEC-6, DEC-7 (USER), DEC-9 (USER), DEC-14 |
...
| **DEC-18** | **Typed children — DEC-11's "C later" half.** What child *types* exist, how each is scored, and how each is rendered so a mechanism or a track is never presented as though it were a research subject | **DEFERRED — P9 half closed, P10 half assigned** | **MIXED** | **P10, before any non-subject child is scored or rendered** | **P10** | P9 v1 now stores an explicit `child_type`; the only emitted value is **`subject`**, and the compact index admits only subject terms. Provenance and segmentation method never imply type. Future enum expansion remains schema-compatible, but `mechanism`/`track` are examples rather than authorized values; P10 must return for the product/rendering choice before emitting either |
