...
> **⚠ Measured defect, 2026-08-17 — some summaries do not describe their span. Prevalence unmeasured.** **Five** spans in `360678` — **7% of that document's 70, and it is the document carrying `(q) Catalysis Science`** — were handed excerpt text belonging to neither their title nor their subject: **`(i) X-Ray Scattering` carries the sentence *"Applications submitted by February 1, 2026, will be considered for funding in FY 2026"***; `(d) Earth-Energy Systems Modeling` carries text trailing off into *"Specific Instructions"*. This is span-boundary construction below — a span begins at its bookmark offset, so it can open mid-sentence inside the *previous* section's prose and a 240-character head then summarizes the wrong thing. **It degrades two consumers at once:** the §11 classifiers judged those spans by the text they were given and rejected them (correctly, for that text), and the same string is what a PI reads on the topic card. Tracked as **Cov5** in §18.1. **Five cases are confirmed by reading, all in `360678` alone** — raised from three when the `claude-sonnet-5` re-baseline surfaced two more: `(k) Plasma Science and Technology—High Energy Density Physics`, handed pre-application review-timeline text, and `(l) Plasma Science and Technology—Microelectronics`, handed relativistic-physics prose from a neighbouring span. A start-of-sentence heuristic was tried as a prevalence estimate and discarded because its output contradicted the observed error pattern, so **prevalence across the rest of the cache remains unmeasured.** Note how these were found: as a by-product of a classifier rejecting them and stating why, not by anyone searching. The true count is **at least** five, and nothing has examined the other 222 spans.
