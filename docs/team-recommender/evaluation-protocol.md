...
Authority: PLAN v2.0 + explicit user amendments E1 and B1. Stage 2 is approved; model/human judgments completed: **0/0**. Marc Porosoff is the sole confirmed human reviewer. One $10 ceiling covers all stages, cumulative $6 through Stage 2 with at least $4 for later stages. The task-specific trusted route remains unavailable. Prior protocol/prompt/schema and the rejected $69 budget are preserved in history. B1 batching replaces the former one-request-per-item cost plan, without changing the semantic rubric, grouped cases or acceptance targets.
