...
| `FF-BUG-025` | Post-3 | High | **Complete in PR #65.** Each award now contributes one most-specific source-namespaced hierarchical program descriptor with parent/child provenance, count-free options, and leaf-query drill-down. Previously, flat aggregation treated a DOE parent office and child program area as two programs, lost hierarchy in drill-down presentation, and exposed loaded-page counts in native options. | `assets/institutional-intelligence*.js` |
