...
| `FF-BUG-005` | 4 | High | **Complete in PRs #68, #69, and #71, with recovery published by PR #72.** Refresh evidence now names each degraded source and failure class, validates publication/cache coherence, and preserves source-specific last-known-good or fail-closed behavior. The recurring JHU failure was traced to interactive Cloudflare challenges across all official unattended routes; JHU is explicitly disabled, publishes zero records, and is never represented as healthy. Issue `#30` closed only after a complete scheduled-equivalent healthy gate. | refresh workflow, source adapters/tools, issue `#30` |
...
| `FF-BUG-010` | 4 | Medium-High | **Complete in PRs #73 and #74.** Alerts use one atomic D1 admission operation keyed by a SHA-256-derived daily client key, while Award and ROR cache misses use per-client-key Cloudflare Durable Objects with independent NSF/NIH/DOE/ROR buckets, bounded `429`/`Retry-After`, and no raw-IP persistence. | alert store, award Worker, deployment config |
...
| `FF-BUG-018` | 4 | Low-Medium | **Complete in PR #73.** Indexed, claim-fenced cleanup applies explicit 30/90/180-day retention classes in batches capped at 500 while preserving active subscriptions, pending/retryable delivery, idempotency, and webhook-correlation evidence; cleanup failure is recorded without blocking delivery. | alert migrations, store, scheduler |
| `FF-BUG-019` | 4 | Medium security hardening | **Complete in PR #73.** New manage and unsubscribe links use purpose-scoped versioned HMAC-SHA256 capabilities verified in constant time with current/previous Worker secrets; D1 stores no reusable raw capability, and legacy links retain a bounded compatibility window through 2026-11-30. | alert schema, crypto, store, email link generation |
| `FF-BUG-020` | 4 | Low | **Complete in PR #73 and operationally hardened in PR #78.** Scheduler rows preserve scheduled time separately from actual injectable-clock start/completion and duration for success, failure, continuation, adoption, timeout, and recovery; health exposes truthful completion and outstanding-window state without subscription content. | alert scheduler, store/schema |
...
| `FF-BUG-023` | Post-3 | High | **Complete in PR #65.** Active award-year criteria are now enforced inclusively on the normalized sequence before paging; one submitted-state snapshot governs later actions, and one immutable UTC clock keeps federal-fiscal-year rollover, NIH requests, and cache identity coherent. Previously, criteria were sent upstream without complete normalized enforcement and later paging/drill-down could read edited-but-unsent form values, admitting out-of-range records, hiding later valid records, or making metrics describe a different request. | award adapters, Funded Awards and Institutional Intelligence controllers |
| `FF-BUG-024` | Post-3 | High | **Complete in PR #65.** Investigator options now use stable conservative labels, deterministic family-name ordering, count-free native options, preserved selection and source variants, and submitted-filter drill-down. Previously, options exposed incomplete loaded-page counts, sorted primarily by count, retained source spelling instead of a stable label, and could lose the selected native-option state after rerender. | `assets/institutional-intelligence*.js` |
| `FF-BUG-025` | Post-3 | High | **Complete in PR #65.** Each award now contributes at most one most-specific source-namespaced hierarchical program descriptor with parent/child provenance, count-free options, and leaf-query drill-down. Previously, flat aggregation treated a DOE parent office and child program area as two programs, lost hierarchy in drill-down presentation, and exposed loaded-page counts in native options. | `assets/institutional-intelligence*.js` |
...
Phase 4 completed on 2026-08-28 through implementation PRs #68, #69, #71, #73, #74, #75, #76, and #78, with the source-recovery publication in [PR #72](https://github.com/mporosoff/grants-scraper/pull/72). The final operational implementation and deployment SHA is `087a248cef7ede5587421150f83c01603f09d45d`; every PR is linked in the execution row below.

- `FF-BUG-005`: PR #68 added source-specific failure classification and bounded retained-data evidence; PR #69 made catalog metadata/publication coherence safe during refresh; PR #71 proved that all official unattended JHU routes were returning an interactive Cloudflare challenge and disabled that source fail-closed rather than claiming health. Scheduled-equivalent [refresh run `33060817728`](https://github.com/mporosoff/grants-scraper/actions/runs/33060817728) passed every enabled external source and the document-evidence layer. Issue [`#30`](https://github.com/mporosoff/grants-scraper/issues/30) closed only after that complete gate, refresh PR #72, and Pages equality succeeded. JHU remains explicitly unavailable, publishes zero records, and is not represented as healthy.
- `FF-BUG-010`: alert admission is one atomic D1 `INSERT ... ON CONFLICT ... WHERE ... RETURNING` operation using a SHA-256-derived daily client key, never a stored raw IP. The Award Worker uses per-client-key Durable Objects with SQLite atomic counters only on cache misses. Independent one-minute limits are 12 for each NSF/NIH/DOE bucket, 60 for ROR search, and 20 for ROR resolve; normal anonymous cache hits do not spend quota, and rejected misses return bounded `429` responses with `Retry-After`.
- `FF-BUG-018`: expired rate-limit rows retain 30 days; evaluation runs and terminal deliveries retain 90 days; provider webhook evidence retains 180 days. Indexed, claim-fenced cleanup defaults to 100 rows per class and is clamped to 500. Active subscriptions, queued/sending/retryable work, provider reconciliation, suppression, and idempotency evidence are never eligible; cleanup failure is recorded but cannot cancel evaluation or delivery.
- `FF-BUG-019`: migration `0004_phase4_alert_operations.sql` adds capability versioning and a legacy compatibility deadline. Version 1 capabilities are stateless, purpose-scoped HMAC-SHA256 tokens for `manage`, `unsubscribe_one`, and `unsubscribe_all`, verified in constant time using required current and previous Worker signing secrets. Only a 16-character key digest is exposed in health. New D1 rows contain no reusable raw manage capability; legacy links expire after `2026-11-30T23:59:59.999Z`, and the repository-managed rotation path fails closed without logging secrets.
- `FF-BUG-020`: actual start/completion clocks and `duration_ms` are distinct from `scheduled_at` for successful and failed runs. Migrations `0005_scheduler_progress.sql` and `0006_scheduler_fencing.sql` add durable stage, heartbeat, frozen input/window, continuation, adoption, claim-token, and revocation evidence without rewriting existing lifecycle rows. Outstanding cursors remain discoverable beyond the 26-hour stale-health horizon, are adopted oldest-first, preserve Sunday weekly eligibility and frozen source boundaries, and are fenced against late or concurrent writes. Health remains unavailable until every required originating window has a truthful terminal success.
- Regression and review: the final PR #78 candidate `406197cc6bbc1e90f11a00ce6d69d486531e3298` passed [exact-head Tests `33194057595`](https://github.com/mporosoff/grants-scraper/actions/runs/33194057595); the terminal [exact-head Codex review comment `5455611378`](https://github.com/mporosoff/grants-scraper/pull/78#issuecomment-5455611378) was clean, and all nine review threads were resolved. [Post-merge Tests `33195495776`](https://github.com/mporosoff/grants-scraper/actions/runs/33195495776) passed 788 Python tests, 406 browser contracts, and 91 Playwright/accessibility tests, with 37 frozen queries showing zero top-10 churn, frozen P9 50/50 and byte-identical, 23 no-drift artifacts unchanged, and zero serious or critical accessibility findings. Targeted alert coverage includes atomic admission, capability compatibility/rotation, retention, scheduler deadlines, continuation and stale recovery, weekly-window ownership, provider quota/idempotency, migration, health, manual execution, and rollback contracts.
- Deployment and production recovery: [Award release `33073212249`](https://github.com/mporosoff/grants-scraper/actions/runs/33073212249) deployed version `61f565cf-0c1b-419d-903c-6534e4ca72ec` with rollback `0edbd7dc-4c17-45e7-acaa-dbead5d10daf`; health, abuse-control, ROR, NSF/NIH/DOE, Pages-equality, and protected-main checks passed. [Alerts release `33195495932`](https://github.com/mporosoff/grants-scraper/actions/runs/33195495932) applied migrations `0005` then `0006`, deployed version `7b95c810-f46f-47a8-9a8d-6100aa75bb34`, retained rollback version `34c69ecb-7c70-4892-a28f-748ece759df0`, and truthfully reported recovery-required HTTP 503 until outstanding production windows drained. [Pages run `33195493725`](https://github.com/mporosoff/grants-scraper/actions/runs/33195493725) published the exact merged SHA.
- Bounded production recovery used the authenticated Cloudflare scheduled-test path with outbound email disabled, so it could not send ordinary subscriber messages. Because the remote scheduled endpoint returned before its background `waitUntil` work was observable, one retry produced two bounded manual daily trigger rows; each terminated in under four seconds, claimed a different oldest outstanding window, and produced no match, delivery, or failure events. The normal five-minute continuation trigger then drained all inherited and newly queued windows oldest-first. At `2026-08-28T18:02:47.959Z`, production `/health` returned HTTP 200 with `service=available`, `delivery_ready=true`, `scheduler_ready=true`, zero stale runs, and zero pending evaluation windows. Aggregate D1 verification found zero running runs, zero incomplete windows, zero provider webhook rows, and only the five pre-existing sent verification events/reservations; no notification event or provider delivery was created by recovery. The repository's bounded health/CORS smoke passed, and no migrations remain pending.
- Search compatibility and release boundary: PR #78 did not change the catalog, vector corpus, model-space fingerprint, ranking thresholds, or default all-record search behavior. It did extend `assets/search-retrieval.js` with an optional candidate-index preparation path so Alerts Strong matching can bound preprocessing to the scheduler-owned candidate set, and coherently updated that asset's hash in `data/search-v2-release.json`. [Search-package run `33195495796`](https://github.com/mporosoff/grants-scraper/actions/runs/33195495796) passed package coherence, the complete Python/browser/frozen-quality gates, Pages equality, and pre/post-publication provider smokes; it deployed Search Worker version `7f2be7a6-96d6-4a22-bf1d-88ba77fda983` with rollback `a0b31bae-d2a2-49c5-ba3b-44c14442896d`. Corpus SHA `499dc6897a1837c3aafcff2e855c92503c300badca49e0aa58846d517c03d6c6` and model-space fingerprint `6bdf01ea5729f7d7a770b8ed4f357537207cc13ba3b95385c63c7a144eb437bd` remained unchanged; the frozen-query and P9 gates prove zero ranking/output churn.
- Release decision: **released and operational**. Existing bounded source scans can still omit upstream history beyond their documented caps, and JHU remains unavailable until an official unattended route can be verified. These are truthful source limitations, not hidden healthy states.

### Post-closeout scheduler architecture item

Before adding any future scheduler functionality, evaluate Cloudflare Workflows and Queues against the verified account plan, workload limits, privacy boundaries, and operating cost. Prefer a Cron-created idempotent Workflow with bounded Queue batches if it materially replaces custom continuation, lease, fencing, timeout, and adoption machinery. D1 must remain the authoritative subscription, baseline, event, suppression, and provider-idempotency store. This is a future architecture decision, not a new active phase of this bug-fix program.
...
The addendum was completed by [PR #65](https://github.com/mporosoff/grants-scraper/pull/65), whose exact reviewed candidate/remediation SHA was `e0ea732129533dda059cc493bd590b00af1594ea` and whose final protected `main` SHA was `192503b7b02aae503a44ee7e04f32785069330b8`. Protected review, exact-head and post-merge tests, the classified Award Worker deployment, retained Alerts Worker classification, Pages deployment, and bounded live closeout all completed successfully; the detailed evidence is recorded below.
...
| Post-Phase-3 Funded Awards / Institutional Intelligence correctness | Complete - 2026-08-26 | [PR #65](https://github.com/mporosoff/grants-scraper/pull/65) | `192503b7b02aae503a44ee7e04f32785069330b8` | Local affected contracts: 35/35. The terminal [exact-head Codex review](https://github.com/mporosoff/grants-scraper/pull/65#issuecomment-5429812028) was clean and all three review threads were resolved. [Exact-head Tests](https://github.com/mporosoff/grants-scraper/actions/runs/33002627306) and [post-merge Tests](https://github.com/mporosoff/grants-scraper/actions/runs/33003624331) passed 778 Python tests, 360 browser tests, and 87 Playwright/accessibility tests, with 37 frozen queries showing zero top-10 churn, frozen P9 50/50 and byte-identical, 23 no-drift artifacts unchanged, and zero serious or critical accessibility findings. | [Award Worker release](https://github.com/mporosoff/grants-scraper/actions/runs/33003624214) deployed version `0edbd7dc-4c17-45e7-acaa-dbead5d10daf` with rollback `31e3e4fc-394c-4ceb-a697-7cdb791cebc0` and NIH adapter `1.4.3`; health, ROR, NSF/NIH/DOE smoke, Pages-equality, and protected-main-stability checks passed. A bounded live NIH request returned `cache=miss` followed by `cache=hit` with the same retrieval timestamp. [Alerts release](https://github.com/mporosoff/grants-scraper/actions/runs/33003624257) classified zero deployment inputs and retained version `d9a02762-b7c9-4fa8-bef3-9edd9e2d8c0e`. [Pages deployment](https://github.com/mporosoff/grants-scraper/actions/runs/33003622675) succeeded. | Released. Bounded upstream scans may not represent complete source history, and source records missing a year cannot be accepted under an active year filter. No catalog, vector, ranking, or search-package files changed, and no refresh workflow ran. |
| Phase 4 - Operational hardening | Complete - 2026-08-28 | [PR #68](https://github.com/mporosoff/grants-scraper/pull/68), [PR #69](https://github.com/mporosoff/grants-scraper/pull/69), [PR #71](https://github.com/mporosoff/grants-scraper/pull/71), [PR #73](https://github.com/mporosoff/grants-scraper/pull/73), [PR #74](https://github.com/mporosoff/grants-scraper/pull/74), [PR #75](https://github.com/mporosoff/grants-scraper/pull/75), [PR #76](https://github.com/mporosoff/grants-scraper/pull/76), and [PR #78](https://github.com/mporosoff/grants-scraper/pull/78); source-recovery publication [PR #72](https://github.com/mporosoff/grants-scraper/pull/72) | `087a248cef7ede5587421150f83c01603f09d45d` (final implementation, migration, deployment, and production recovery) | Final candidate [Tests `33194057595`](https://github.com/mporosoff/grants-scraper/actions/runs/33194057595), clean [Codex review `5455611378`](https://github.com/mporosoff/grants-scraper/pull/78#issuecomment-5455611378), and [post-merge Tests `33195495776`](https://github.com/mporosoff/grants-scraper/actions/runs/33195495776) passed. The protected gate comprised 788 Python, 406 browser, and 91 Playwright/accessibility tests; 37 frozen queries had zero top-10 churn, P9 was 50/50 and byte-identical, 23 no-drift artifacts were unchanged, and accessibility reported zero serious or critical findings. | [Award release `33073212249`](https://github.com/mporosoff/grants-scraper/actions/runs/33073212249): version `61f565cf-0c1b-419d-903c-6534e4ca72ec`, rollback `0edbd7dc-4c17-45e7-acaa-dbead5d10daf`. [Alerts release `33195495932`](https://github.com/mporosoff/grants-scraper/actions/runs/33195495932): migrations `0005`/`0006`, version `7b95c810-f46f-47a8-9a8d-6100aa75bb34`, rollback `34c69ecb-7c70-4892-a28f-748ece759df0`. [Search-package release `33195495796`](https://github.com/mporosoff/grants-scraper/actions/runs/33195495796): version `7f2be7a6-96d6-4a22-bf1d-88ba77fda983`, rollback `a0b31bae-d2a2-49c5-ba3b-44c14442896d`. [Pages `33195493725`](https://github.com/mporosoff/grants-scraper/actions/runs/33195493725) succeeded. Production recovery ended at HTTP 200 with `scheduler_ready=true`, zero pending windows, zero running runs, and no recovery-created notification or provider delivery. | Released. Issue #30 is closed after scheduled-equivalent run `33060817728`; JHU remains explicitly disabled/fail-closed because official unattended routes require an interactive Cloudflare challenge. PR #78 added only an optional candidate-preparation path to shared search retrieval plus its coherent release hash; the catalog, vectors, corpus/model fingerprints, ranking thresholds, and default output remained unchanged, with zero frozen-query/P9 churn. Future scheduler additions require a separate Workflows/Queues architecture evaluation. |
...
### Post-Phase-3 correctness regression and verification evidence

- `FF-BUG-023`: inclusive normalized year enforcement now precedes paging; submitted state is preserved across later actions; one immutable UTC clock governs federal-fiscal-year rollover, NIH request construction, and start-only request/cache identity. Deterministic regressions cover the pre- and post-rollover cache miss/hit paths, one-clock coherence, stable unaffected cache keys, missing-year rejection, and later valid normalized records.
- `FF-BUG-024`: investigator labels are stable and conservative, options sort deterministically by family name, native options are count-free, selected state and accessible source variants persist, and drill-down uses the exact submitted filters. Contract and real-browser coverage retain the conservative identity/conflict rules and the three-award Marc result.
- `FF-BUG-025`: every award contributes at most one most-specific source-namespaced hierarchical program descriptor; parent/child and source-code provenance remain visible, native options are count-free, and selection uses the leaf query while retaining submitted filters. Mixed-source fixtures and Playwright cover distinct children, parent fallback, cross-source separation, hierarchy presentation, and drill-down.
...
The Unit D candidate record, which becomes final only upon protected merge, preserves the remaining boundaries: JHU's official unattended routes remain blocked by an interactive Cloudflare challenge; complete-result snapshots are immutable and server-authoritative but cached for 3,600 seconds with Cloudflare datacenter locality; provider acceptance is not authorized inbox receipt; and user-connected AI keys remain local and user-managed. Cloudflare Workflows/Queues remains separate future scheduler architecture work, not an incomplete phase of this program.
